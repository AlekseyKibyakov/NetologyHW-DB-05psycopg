...
def test(conn):
    create_db(conn)
    client1 = Client(conn, "Alex", "Cyb", "alex@gmail.com", "79999999987")
    client2 = Client(conn, "Mya", "Mua", "mya@mail.ru", "79111111111")
    ...
    client2.change_client_info(conn, name='Kau', surname='Kyu', email='kau@mail.ru', phone='79781123125', phone_to_change='79111111111')

    client1.del_phone(conn, "79999999987")

    # client2.del_client(conn)

    find_client(conn, name="alex")
    find_client(conn, phone="71111111177")
    find_client(conn, name ='kau', phone='79781123125')



with psycopg2.connect(database = '05psycopg', user = 'postgres', password = '1') as conn:
    test(conn)
conn.close()